...
def test_fact_claims_required_columns(spark):
    df = spark.table(TBL)
    missing = [c for c in REQUIRED_COLS if c not in df.columns]
    assert not missing, f"fact_claims missing columns: {missing}"

def test_fact_claims_pk_not_null(spark):
    df = spark.table(TBL)
    assert df.filter(F.col("Claim_ID").isNull()).count() == 0

def test_fact_claims_pk_unique(spark):
    df = spark.table(TBL)
    total = df.count()
...
def test_fact_claims_dq_flags_binary(spark):
    df = spark.table(TBL)
